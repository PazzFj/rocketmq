...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {    // 服务状态（默认刚创建）
            case CREATE_JUST:
                ...
                //创建mq客户实例  MQClientManager  => 创建 MQClientInstance
                this.mQClientFactory = MQClientManager.getInstance().getAndCreateMQClientInstance(this.defaultMQProducer, rpcHook); // 创建mq客户实例
                ...
                if (!registerOK) {  // 若生成组名称已存在, 抛异常
                    ...
                    throw new MQClientException("The producer group[" + this.defaultMQProducer.getProducerGroup() + "] has been created before, " +
                            "specify another name please." + FAQUrl.suggestTodo(FAQUrl.GROUP_NAME_DUPLICATE_URL), null);
                }
    ...
    private SendResult sendDefaultImpl(Message msg, final CommunicationMode communicationMode, final SendCallback sendCallback, final long timeout)
        ...
        // 获取PublishInfo 信息
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic()); // 找到 TopicPublishInfo 对象
        if (topicPublishInfo != null && topicPublishInfo.ok()) {    // 消息队列不为空
            ...
            for (; times < timesTotal; times++) {
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        //发送
                        sendResult = this.sendKernelImpl(msg, mq, communicationMode, sendCallback, topicPublishInfo, timeout - costTime); //核心 发送
    ...
    /**
     * 尝试去寻找TopicPublishInfo
     */
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        TopicPublishInfo topicPublishInfo = this.topicPublishInfoTable.get(topic);
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            this.mQClientFactory.updateTopicRouteInfoFromNameServer(topic); //创建TopicPublishInfo 时, 并根据topic修改TopicRouteInfo 路线信息
            topicPublishInfo = this.topicPublishInfoTable.get(topic);   // 重新获取 PublishInfo (这时候已更新TopicPublishInfo   有MessageQueue & TopicRouteData)
        }
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (brokerAddr != null) {
            ...
            try {
                //对于MessageBatch, 在生成过程中已经设置了ID
                if (!(msg instanceof MessageBatch)) {
                    // message 创建UNIQ_KEY 生成ID
                    MessageClientIDSetter.setUniqID(msg);
                }

                int sysFlag = 0;
                boolean msgBodyCompressed = false;
                if (this.tryToCompressMessage(msg)) {       // false 如果message 中 body 长度超过 1024 * 4 则为true
                ...
                // 创建 发送消息请求头  SendMessageRequestHeader
                ...
                requestHeader.setFlag(msg.getFlag());       // 0
                ...
                switch (communicationMode) {
                    case ASYNC:
                        ...
                        if (msgBodyCompressed) {
                            // 如果压缩了msg体，则应使用prevBody重置msgbody
                            // 克隆新消息使用压缩的消息体和恢复原始按摩
                        ...
                        //核心, 发送
                        sendResult = this.mQClientFactory.getMQClientAPIImpl().sendMessage(brokerAddr, mq.getBrokerName(),  //核心, 异步发送
                                tmpMessage, requestHeader, timeout - costTimeAsync, communicationMode, sendCallback, topicPublishInfo,
                    ...
                    case SYNC:
                        ...
                        sendResult = this.mQClientFactory.getMQClientAPIImpl().sendMessage(brokerAddr, mq.getBrokerName(),
                                msg, requestHeader, timeout - costTimeSync, communicationMode, context, this);
                        break;
                    default:
                        assert false;
                        break;
